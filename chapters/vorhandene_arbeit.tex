...
\autoref{alg:region-growing} beschreibt den Ablauf bei maximaler Krümmung $\phi$, Normalendifferenz $\alpha$ und unter Betrachtung der $n \gets$ \ac{KNN}.
...
\ForAll{$p \in P$}
	...
	\ForAll{unprocessed points $q \in \mathcal{Q}$}
		\State mark $q$ as processed
		\State $\mathcal{N} \gets$ \Call{getKNeighbors}{$q, n$}
		\Comment{$n$ nearest neighbors of $q$}
...
\begin{algorithm}
\caption[Delaunay-Triangulierung]{Delaunay-Triangulierung nach \cite[200--201]{berg2000comp}}
...
\Function{checkFlipped}{new vertex $p$, edge $t$}
	\If{the smallest angle is bigger after a flip}
		...
		\State remove $t$
		\State check untouched edges not ending at $p$ using \Call{checkFlipped}{$p$, edge}
	\EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
